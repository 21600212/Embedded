...
ReceiverIR::ReceiverIR(PinName rxpin) : evt(rxpin) {
    ...
    evt.mode(PullUp);
    uint64_t tmp = 10000;
    ticker.attach(callback(this, &ReceiverIR::isr_wdt), tmp);
}
...
void ReceiverIR::isr_fall(void) { //ppt그림에서 똑 떨어지는 시점.
    LOCK();
    switch (work.state) {
        case Idle:
            if (work.c1 < 0) {
        ...
        case Receiving:
            if (RemoteIR::NEC == data.format) {
                ...
                if (InRange(a, RemoteIR::TUS_NEC * 3)) { //3T인 경우 (==1)
                    data.buffer[data.bitcount / 8] |= (1 << (data.bitcount % 8)); //buffer를 1로 만들겠다.
                } else if (InRange(a, RemoteIR::TUS_NEC * 1)) { //1T인 경우(==0)
                    data.buffer[data.bitcount / 8] &= ~(1 << (data.bitcount % 8)); //buffer를 0으로 만들겠다.
                }
                data.bitcount++; //buffer에 차는 bitcount구나!
#if 0
                /*
                 ...
                 */
                timeout.detach();
                timeout.attach(callback(this, &ReceiverIR::isr_timeout), RemoteIR::TUS_NEC * 5); //5*TUS_NEC만큼 timeout
#endif
...
void ReceiverIR::isr_rise(void) { //burst가 있다.
    LOCK();
    switch (work.state) {
        case Idle:
            if (0 <= work.c1) {
                ...
                int a = work.c2 - work.c1;
                if (InRange(a, RemoteIR::TUS_SONY * 4)) { //SONY기준의 4T만큼이면
                    data.format = RemoteIR::SONY;           //포맷은 SONY이다.
                    work.state = Receiving;
                    data.bitcount = 0;
                } else {
