...
public:
	 ...
	 */
	Adafruit_SSD1306_Spi(PinName MOSI, PinName SCLK, PinName CS, PinName DC, PinName RST,  
	                     uint8_t rawHieght = 64, uint8_t rawWidth = 128)
	    : Adafruit_SSD1306(RST, rawHieght, rawWidth)
	    , cs(CS,true)
	    , dc(DC,false)
	    , oledSpi(MOSI, NC, SCLK)
	    {
	    	oledSpi.format(8, 0);      // 8 bits, mode 0  (default value)
	    	oledSpi.frequency(5000000);   // 1 MHz (default value)
...
protected:
	virtual void sendDisplayBuffer()
	{
		...
		for(std::vector<uint8_t>::size_type i=0, q=buffer.size(); i<q; i++)
			oledSpi.write(buffer[i]);
/*
		if(height() == 32)
		{
